...
Do **not** merge PR #254 as-is. After PR #255/#258, `origin/main` now locks
`docs/doctrine/CRAVE_FRONTEND_EXECUTION_ORDER.md` as the controlling frontend
order: Foundation Gate → Place Detail proving slice → Feed/Decision Session →
Search/Map propagation-only. This visual shell branch predates that order and
would reintroduce a conflicting execution path if merged directly.

Preserve this branch as a reference/prototype unless the work is explicitly
split or re-scoped under the locked order. Any future Search/Map work must
respect the doctrine's Search/Map boundary: propagate shared contracts,
reliability, data-state conventions, auth/error/query ownership,
accessibility fixes, and integration hardening only — no UX redesign without
a new proven contract gap.
