"""
Coverage for toast.py's _fetch_with_playwright -- same Chromium-leak
class fixed in browser_fallback.py. sync_playwright is imported inside
the function itself, so it's patched at its real source
...
from app.services.menu.extraction.provider.handlers.toast import _fetch_with_playwright
...
def test_browser_closed_after_success():
    ...
    page.content.return_value = "x" * 2000
    with patch("playwright.sync_api.sync_playwright", return_value=cm):
        result = _fetch_with_playwright("https://toasttab.com/x")
