...
def get_user_rankings(
    ...
    """Compatibility endpoint for an owner's own list only.

    Exact Rank positions and full ordered lists are private by default.
    Public identity visibility never grants access to this data.
    """
    profile = (
        db.query(UserProfile).filter(UserProfile.id == target_user_id).one_or_none()
    )
    if not profile or user_id != target_user_id:
