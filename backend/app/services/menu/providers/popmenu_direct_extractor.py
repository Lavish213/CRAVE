...
def extract_popmenu_direct(url: str) -> List[ExtractedMenuItem]:
    ...
    try:
        ...
        logger.debug("popmenu_direct_failed url=%s error=%s", url, exc)

    logger.info("popmenu_direct_items=%s url=%s", len(items), url)
