...
from ingest.sources.arcgis_fetch import fetch_arcgis_dataset
...
def _coord_valid(lat: Optional[float], lng: Optional[float]) -> bool:
...
def _get_lng(row: Dict[str, Any], config: Any) -> Optional[float]:

    fields = [
        getattr(config, "lng_field", None),
        "_geometry_x",
        "longitude",
        "Longitude",
        "LONGITUDE",
...
def _get_row_id(row: Dict[str, Any]) -> Optional[str]:
...
def _get_permit_id(row: Dict[str, Any], config: Any) -> Optional[str]:

    field_name = getattr(config, "permit_id_field", None)
...
def _get_facility_id(row: Dict[str, Any], config: Any) -> Optional[str]:

    field_name = getattr(config, "facility_id_field", None)
...
def _build_external_id(
    *,
    ...
    if name and address:
        return f"{source}:{city_slug}:synthetic:{_slugify_for_id(name)}:{_slugify_for_id(address)}"
...
def _row_is_active(row: Dict[str, Any], config: Any) -> bool:
    ...
    if status_active_values:
        ...
        return lowered in normalized_allowed
    ...
    return lowered not in inactive_tokens
...
def _normalize_rows(
    *,
    ...
    dataset_id = (
        _clean(getattr(config, "dataset_id", None))
        or _clean(getattr(config, "city_slug", None))
        or "arcgis"
    )
    ...
    for row in rows:
        ...
        try:
            ...
            lat = _get_lat(row, config)
            lng = _get_lng(row, config)

            has_coords = _coord_valid(lat, lng)
            ...
            phone = _normalize_phone(row.get(getattr(config, "phone_field", None)))

            website = _normalize_website(row.get(getattr(config, "website_field", None)))

            permit_id = _get_permit_id(row, config)
            facility_id = _get_facility_id(row, config)
            ...
            logger.debug(
                "arcgis_normalization_failed city=%s error=%s",
                getattr(config, "city_slug", "unknown"),
                exc,
            )

    logger.info(
        "arcgis_normalization_summary city=%s kept=%s skipped_inactive=%s skipped_invalid=%s skipped_duplicates=%s skipped_missing_identity=%s skipped_bad_coords=%s",
        getattr(config, "city_slug", "unknown"),
        len(normalized),
        skipped_inactive,
        skipped_invalid,
...
def run_arcgis_ingest(city_slug: str) -> None:

    city_slug = _canonical_city_slug(city_slug)

    logger.info("arcgis_ingest_start city=%s", city_slug)

    config = get_health_dataset(city_slug)

    dataset_type = _clean(getattr(config, "dataset_type", None))

    if dataset_type and dataset_type.lower() != "arcgis":

        raise RuntimeError(
            f"City '{city_slug}' dataset_type is '{dataset_type}', not arcgis"
        )

    base_url = (
        _clean(getattr(config, "arcgis_url", None))
        or _clean(getattr(config, "base_url", None))
    )

    if not base_url:

        raise RuntimeError(
            f"ArcGIS dataset URL missing for city '{city_slug}'"
        )

    logger.info(
        "arcgis_dataset_config city=%s dataset=%s",
        city_slug,
        _clean(getattr(config, "dataset_id", None)) or "arcgis",
    )

    start = time.time()

    rows = fetch_arcgis_dataset(
        base_url=base_url,
    ...
    logger.info(
        "arcgis_rows_fetched city=%s count=%s seconds=%s",
        city_slug,
        len(rows),
        elapsed,
    )
    ...
    if len(rows) > MAX_ROWS_PROTECTION:
        ...
        raise RuntimeError(
            f"ArcGIS dataset too large ({len(rows)} rows) limit={MAX_ROWS_PROTECTION}"
        )

    if not rows:

        logger.warning("arcgis_rows_empty city=%s", city_slug)

        return

    rows = sanitize_health_rows(rows=rows, config=config)

    logger.info(
        "arcgis_rows_sanitized city=%s count=%s",
        city_slug,
        len(rows),
    )
    ...
    if not rows:
        ...
        logger.warning("arcgis_rows_sanitized_empty city=%s", city_slug)
    ...
    logger.info(
        "arcgis_rows_normalized city=%s count=%s",
        city_slug,
        len(normalized),
    )
    ...
    if not normalized:
        ...
        logger.warning("arcgis_rows_normalized_empty city=%s", city_slug)
    ...
    logger.info(
        "arcgis_checkpoint_written city=%s path=%s",
        city_slug,
        checkpoint,
    )
    ...
    try:
        ...
        logger.info("arcgis_promotion_complete city=%s", city_slug)

    except Exception as exc:

        logger.exception(
            "arcgis_promotion_failed city=%s error=%s",
            city_slug,
            exc,
        )
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage:")
        print("python scripts/run_arcgis_ingest.py <city_slug>")
