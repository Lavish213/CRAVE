...
def _row_is_active(row: Dict[str, Any], config: Any) -> bool:
    ...
    if status_active_values:
        normalized_allowed = {str(v).strip().lower() for v in status_active_values}
    ...
    *,
    ...
    for row in rows:
        try:
            ...
            name = _normalize_name(row.get(config.name_field))
            if not name:
                skipped_missing_identity += 1
                continue

            lat = _safe_float(row.get(config.lat_field)) if getattr(config, "lat_field", None) else None
            lng = _safe_float(row.get(config.lng_field)) if getattr(config, "lng_field", None) else None
            ...
            phone = _normalize_phone(
                row.get(config.phone_field) if getattr(config, "phone_field", None) else None
            )

            website = _normalize_website(
                row.get(config.website_field) if getattr(config, "website_field", None) else None
            )

            permit_id = _clean(
                row.get(config.permit_id_field)
            ) if getattr(config, "permit_id_field", None) else None

            facility_id = _clean(
                row.get(config.facility_id_field)
            ) if getattr(config, "facility_id_field", None) else None
...
def run_socrata_ingest(city_slug: str) -> None:
    ...
    rows = fetch_socrata_dataset(
        ...
        where=getattr(config, "where", None),
    ...
    logger.info("health_rows_fetched city=%s count=%s", city_slug, len(rows))
